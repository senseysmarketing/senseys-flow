-- Desativar mapeamentos do status "Qualificado" pois agora o evento é disparado pela temperatura "hot"
UPDATE meta_event_mappings 
SET is_active = false 
WHERE status_id IN (SELECT id FROM lead_status WHERE name = 'Qualificado');

-- Atualizar a função de criação de mapeamentos padrão para não criar mapeamento para "Qualificado"
...
BEGIN
  ...
  -- NOTA: "Qualificado" não dispara evento automático
  -- O evento "Lead (qualified)" é disparado automaticamente quando temperatura = "hot"

  -- Visita Agendada -> Schedule event
  SELECT id INTO v_status_id FROM public.lead_status 
  ...
  -- Proposta -> SubmitApplication event
  SELECT id INTO v_status_id FROM public.lead_status 
  ...
  -- Fechado -> Purchase (converted) event
  SELECT id INTO v_status_id FROM public.lead_status 
