    ...
    try {
      ...
      for (const entry of data.entry || []) {
        ...
        for (const change of entry.changes || []) {
          ...
          // Note: CAPI events are now triggered only when lead status changes in the CRM
          // Temperature is used for visual prioritization, not for automatic event dispatch
          console.log(`ℹ️ Lead temperature: ${temperature} (score: ${score}) - CAPI event will be sent when status changes`);
        }
      }
